...
router = APIRouter()
logger = logging.getLogger("coepd-api.pages")
...
def register_page_routes(templates: Jinja2Templates) -> APIRouter:
    ...
    async def home(request: Request):
        try:
            ...
            logger.exception("Failed rendering home page template: %s", exc)
            return HTMLResponse(
                content=(
                    "<!doctype html><html><head><meta charset='utf-8'>"
                    "<meta name='viewport' content='width=device-width, initial-scale=1'>"
                    "<title>COEPD</title></head><body>"
                    "<h1>COEPD</h1><p>Website is starting up. Please refresh in a moment.</p>"
                    "</body></html>"
                ),
                status_code=200,
            )
